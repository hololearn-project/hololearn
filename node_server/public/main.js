...
let raycaster = new THREE.Raycaster({antialias: true});
...
function updateMarker() {
  vertMarker.position.x = -facePositions[(markerCount*3)];
  vertMarker.position.y = facePositions[(markerCount*3)+1] + 50;
  vertMarker.position.z = facePositions[(markerCount*3)+2];
...
async function load3DEnvironment() {
  ...
  function animate() {
    ...
    for (k = 0; k < FACE_MESH_LANDMARK_COUNT; k++) {
      facePositions[(3*k)] = facialLandmarks[0][mapping[k]].x*100;
      facePositions[(3*k)+1] = -facialLandmarks[0][mapping[k]].y*100;
      facePositions[(3*k)+2] = facialLandmarks[0][mapping[k]].z*100;
    }
  ...
  if (table >= 0) {
    ...
    document.getElementById('markerBtn').style.display = 'block';
